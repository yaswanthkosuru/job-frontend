...
import { getJobApplicationAnalytics, useJobApplicationAnalytics } from "@/features/jobapplicants/jobapplicantslice";
...
export default function InterviewerPage() {
  ...
  return (
    <div className="container mx-auto py-10 px-4 md:px-6 lg:px-8"> {/* Added more padding */}
      <h1 className="text-3xl md:text-4xl font-bold mb-10 text-center md:text-left"> {/* Increased margin, responsive text */}
        Interview Dashboard
      </h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8"> {/* Adjusted grid breakpoints, increased gap */}
        {jobApplicationAnalytics.map((position) => (
          <Card key={position.id} className="hover:shadow-lg transition-shadow duration-200 ease-in-out flex flex-col"> {/* Added flex flex-col for equal height button positioning */}
            <CardHeader className="pb-4"> {/* Adjusted padding */}
              ...
              <CardDescription className="text-sm text-muted-foreground flex items-center gap-2 pt-1">
                ...
                <span>
                  {position.application_count} {position.application_count === 1 ? 'application' : 'applications'} pending interview
                </span>
              </CardDescription>
            </CardHeader>
            <CardContent className="flex-grow pt-2 pb-4"> {/* flex-grow pushes footer down, adjusted padding */}
            ...
            <CardFooter className="pt-4 border-t"> {/* Added border-top for separation, padding */}
               <div className="flex justify-between items-center w-full">
                 {/* Moved Button to Footer for consistent bottom alignment */}
                 <Button size="sm" className="w-full justify-between"> {/* Adjusted size */}
                    View Applications

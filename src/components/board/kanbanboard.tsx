...
export default function ApplicationBoard() {
  ...
  const {
    jobapplicants: applications,
    status,
    error,
  } = useJobApplicants({ job_id: Number(id) });
  ...
  const handleSelectedInterviewers = (userId: number) => {
    dispatch(
      scheduleInterview({
        jobapplicant_id: Number(selectedId),
        user_id: userId,
      })
    );
    console.log(userId);
        ...
        <AccordionItem value="item-1">
          ...
          <AccordionContent>
            ...
            <p className="text-muted-foreground">
              Double tap to see candidate details
            </p>
          </AccordionContent>
        </AccordionItem>
      </Accordion>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {statuses.map((status, statusIndex) => (
          <div key={status} className="flex flex-col">
            ...
            <div
              className="bg-muted/50 rounded-lg p-4 min-h-[70vh] flex flex-col gap-4"
              id={String(statusIndex)}
            >
              {applications
                ...
                .map((application) => (
                  <motion.div
                    ...
                    onDragEnd={(_, info) => {
                      ...
                      const targetColumn = Math.floor(
                        (info.point.x + window.scrollX) / columnWidth
                      );
                        ...
                        <div className="flex-1 min-w-0">
                          {application.status === "pending" && (
                            <div
                              ...
                              className={`h-6 w-6 cursor-pointer bg-gray-100 rounded-md flex justify-center items-center ${
                                statusColors[application.status]
                              }`}
                            >
                              <UserPlus />
                          ...
                          <p className="text-sm text-muted-foreground truncate">
                            {application.candidate.job_title}
                          </p>
                    ...
                    <div className="md:w-1/3">
                      <Avatar className="h-24 w-24 mb-4">
                        <AvatarFallback className="bg-primary text-primary-foreground text-2xl">
                          {getInitials(
                            selectedCandidate.candidate.user.username
                          )}
                        </AvatarFallback>
                      </Avatar>
                      ...
                      <p className="text-muted-foreground">
                        {selectedCandidate.candidate.job_title}
                      </p>
                      ...
                      <div className="mt-4 space-y-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm">
                            {selectedCandidate.candidate.user.email}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Phone className="h-4 w-4 text-muted-foreground" />
                          <span className="text-sm">
                            {selectedCandidate.candidate.user.phone}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                        ...
                        <TabsContent value="details" className="space-y-4">
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {selectedCandidate.candidate.skills.map(
                                (skill) => (
                          ...
                          <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              ...
                              <div className="bg-muted/50 p-3 rounded-md">
                                ...
                                <p>
                                  {formatSalary(
                                    selectedCandidate.candidate.expected_salary
                                  )}
                                </p>
                              </div>
                              <div className="bg-muted/50 p-3 rounded-md">
                                ...
                                <p>
                                  {selectedCandidate.candidate.notice_period}{" "}
                                  days
                                </p>
                              </div>
                            </div>
                        ...
                        <TabsContent value="experience">
                          <div className="border-l-2 border-muted pl-4 py-2">
                            <div className="relative">
                              ...
                              <p className="text-sm">
                                {selectedCandidate.candidate.company}
                              </p>
                              <p className="text-xs text-muted-foreground">
                                {formatDate(
                                  selectedCandidate.candidate.start_date
                                )}{" "}
                                -{" "}
                        ...
                        <TabsContent value="education">
                          <div className="border-l-2 border-muted pl-4 py-2">
                            <div className="relative">
                              <div className="absolute -left-6 mt-1 h-4 w-4 rounded-full bg-primary"></div>
                              <h3 className="font-medium">
                                {selectedCandidate.candidate.degree}
                              </h3>
                              <p className="text-sm">
                                {selectedCandidate.candidate.institution}
                              </p>
                              <p className="text-xs text-muted-foreground">
                                Graduated{" "}
                                {selectedCandidate.candidate.year_of_completion}
                              </p>
                            </div>
                          </div>
                        </TabsContent>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[600px] h-[90vh]">
          ...
          <div className="h-[calc(100vh-200px)] overflow-y-auto p-4">
            <SelectInterviewer
              onSelectedInterviewers={handleSelectedInterviewers}
            />

...
import { Mail, Phone, Linkedin, Github, Briefcase, GraduationCap, FileText, Sparkles } from "lucide-react";
      ...
      <Card className="relative max-w-lg mx-auto p-6 rounded-2xl shadow-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100">
        <Dialog open={isAiDialogOpen} onOpenChange={setIsAiDialogOpen}>
          ...
          <DialogContent className="max-w-3xl max-h-[90vh]">
            <DialogHeader>
              <DialogTitle className="text-xl">Summary for {user.user.username}</DialogTitle>
            </DialogHeader>
        ...
        <CardContent className="flex flex-col items-center gap-4">
          <Avatar className="w-24 h-24">
            <AvatarImage src={getAvatarUrl(user.user.email)} alt={user.user.username} />
            <AvatarFallback>{user.user.username.charAt(0)}</AvatarFallback>
          </Avatar>
          <h2 className="text-2xl font-semibold">{user.user.username}</h2>
          <p className="text-gray-500">{user.job_title} at {user.company}</p>
          <div className="flex flex-wrap justify-center gap-4">
            <Button variant="outline" asChild>
              <a href={`mailto:${user.user.email}`}><Mail className="mr-2" size={16} />Email</a>
            </Button>
            <Button variant="outline" asChild>
              <a href={`tel:${user.user.phone}`}><Phone className="mr-2" size={16} />Call</a>
            </Button>
            <Button variant="outline" asChild>
          ...
          <div className="mt-4 w-full">
            <Button className="mt-4 w-full" asChild>
              <a href={user.resume_file_url} target="_blank" rel="noopener noreferrer">

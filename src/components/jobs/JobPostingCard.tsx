...
import { Building2, MapPin,IndianRupee, Layers, Clock, Pencil } from "lucide-react";
...
export const JobPostingCard = ({ job }: JobPostingCardProps) => {
  ...
  const handleSubmit = async (data: JobPostingFormValues) => {
    await dispatch(updateJobPosting({
      ...
      jobPostingData: {
        ...data,
        employment_type: data.employment_type as "full_time" | "part_time" | "contract" | "internship"
      }
    }));
    setOpen(false);
  };
      ...
      <Card className="group hover:shadow-lg transition-all duration-300">
        ...
        <CardContent className="grid grid-cols-2 gap-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              <p className="text-sm">
                {job.recruiter.organisation.name} • {job.recruiter.organisation.location}
              </p>
            </div>
            <div className="flex items-center space-x-2">

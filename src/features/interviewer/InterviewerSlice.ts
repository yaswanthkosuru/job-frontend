...
export const fetchInterviewers = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    ...
    try {
      const response = await axios.get<InterviewerProfile[]>(
        url,
        getAuthHeaders()
      );
...
export const createInterviewer = createAsyncThunk(
  ...
  async (interviewerData: InterviewerProfile) => {
    const response = await axios.post<InterviewerProfile>(
      `${API_URL}/api/v1/user/interviewer/`,
      interviewerData,
      getAuthHeaders()
    );
    return response.data;
...
export const useInterviewerSelector = () => {
  const interviewerData = useSelector(
    (state: { interviewer: InterviewerState }) => state.interviewer
  );
